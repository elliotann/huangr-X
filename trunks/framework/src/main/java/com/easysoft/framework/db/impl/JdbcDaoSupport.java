...
public class JdbcDaoSupport<T> implements IDaoSupport<T> {
	...
	public int getLastId(String table) {
		if ("2".equals("1")) {
            ...
            try {
                ...
                String id = null;
                while(rs.next()){
                    id = (String)rs.getObject(4);
                }
                return this.jdbcTemplate
                        .queryForInt("SELECT max("+id+") as id from " + table);
	...
	public List<T> queryForList(String sql, Class clazz, Object... args) {
		return this.jdbcTemplate.query(sql, args, ParameterizedBeanPropertyRowMapper.newInstance(clazz));
	}
	...
	public T queryForObject(String sql, Class clazz, Object... args) {
		try {
			return (T) jdbcTemplate
					.queryForObject(sql, args, ParameterizedBeanPropertyRowMapper
							.newInstance(clazz));
	...
	public T queryForObject(String sql, ParameterizedRowMapper mapper,
			Object... args) {
		try {
			T t = (T) this.jdbcTemplate.queryForObject(sql,args,mapper);
			return t;
		} catch (RuntimeException e) {
			return null;
		}
