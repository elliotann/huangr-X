...
public class MenuJsonGetter extends AbstractFacadeProcessor {
    ...
    public  String getAuthMenuJson(String roleId){
        ...
        json.append("'app':[");
        json.append(getToJsonAuth(applist,authList));
        json.append("]");

        json.append(",'ext':[");
        json.append(getToJsonAuth(extlist,authList));
        json.append("]");
        json.append("};");
    ...
    public  String getToJsonAuth(List<Menu> menuList,List<AuthAction> authList){
        ...
        for(Menu menu:menuList ){
            ...
            menuItem.append( toJsonAuth (menu,authList));
            i++;
     ...
     * @return
     */
    private  String toJsonAuth(Menu menu,List<AuthAction> authList){
        ...
        if(menu.getChildren().size()>0){
            ...
            menuItem.append(getChildrenJsonAuth(menu.getId(), menu.getChildren(),authList));
        }
     ...
     * @return
     */
    private  String  getChildrenJsonAuth(Integer menuId,List<Menu> menuList,List<AuthAction> authList){
        StringBuffer json = new StringBuffer();
        json.append("[");
        int i=0;
        for (Menu menu : menuList) {
            ...
            if(i!=0)
                json.append(",");
            json.append(toJsonAuth(menu, authList));
            i++;
        }
    ...
    public  String getoJson(List<Menu> menuList,List<Menu> allList,List<AuthAction> authList){
        StringBuffer menuItem = new StringBuffer();
        int i =0;
