...
<ul>
	...
	<li><a href="themeFile!toNewFile.do?themeid=${themeid }&folderName=${folderName}&type=${type}&themeType=${themeType}" id="newFileBtn">新建文件</a></li>
...
<table>
	...
	<c:forEach items="${fileList}" var="file" >
		<tr>
			...
			<a href="themeFile!list.do?themeid=${themeid }&folderName=${folderName}${file.name}/&type=${type}&themeType=${themeType}" >${file.name }</a>
			</c:if>
			...
			<c:if test="${file.isfolder==0 && file.isImage==0}"  >
				<a href="themeFile!toEdit.do?themeid=${themeid }&folderName=${folderName}&name=${file.name}&type=${type}&themeType=${themeType}" >编辑</a>
				&nbsp;&nbsp;
			</c:if>
			
			<a href="javascript:;" name="${file.name }" class="rename">改名</a>
			&nbsp;&nbsp;
			<a href="themeFile!delete.do?themeid=${themeid }&folderName=${folderName}&name=${file.name}&type=${type}&themeType=${themeType}" onclick="javascript:return confirm('确定删除此文件吗？删除后不可恢复');" class="del">删除</a>
			<c:if test="${file.isfolder==0}"  >
			&nbsp;&nbsp;
