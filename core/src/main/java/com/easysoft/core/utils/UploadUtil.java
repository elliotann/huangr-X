...
public class UploadUtil {
	...
	public static String uploadUseRequest(String name,String subFolder){
		...
		try {
			String encoding = ParamSetting.ENCODING;
			...
			fileName = DateUtil.toString(new Date(), "yyyyMMddHHmmss") + com.easysoft.framework.utils.StringUtil.getRandStr(4) + "." + ext;
			
			filePath =  ParamSetting.IMG_SERVER_PATH + EsfContext.getContext().getContextPath()+  "/attachment/";
			if(subFolder!=null){
				filePath+=subFolder +"/";
			}
			
			String path  = ParamSetting.FILE_STORE_PREFIX+ "/attachment/" +(subFolder==null?"":subFolder)+ "/"+fileName;
	...
	public static String[] upload(File file,String fileFileName,String subFolder,int width,int height ){
		...
		filePath =  ParamSetting.IMG_SERVER_PATH+ getContextFolder()+ "/attachment/";
		if(subFolder!=null){
			filePath+=subFolder +"/";
		}
		
		path[0]  = ParamSetting.IMG_SERVER_DOMAIN+ getContextFolder()+ "/attachment/" +(subFolder==null?"":subFolder)+ "/"+fileName;
		filePath += fileName;
		FileUtil.createFile(file, filePath);
	...
	public static void deleteFile(String filePath){
		filePath =filePath.replaceAll(ParamSetting.IMG_SERVER_DOMAIN, ParamSetting.IMG_SERVER_PATH);
	...
	private static String getContextFolder(){
		...
		return EsfContext.getContext().getContextPath();
	}
