...
public class FileUploader
{
    ...
    private Set<String> acceptTypes = new HashSet<String>();
    ...
    /**
     * @param savePath 上传文件保存路径，不包含文件名
     * @param acceptTypes 可接受上传文件类型，不传不限制
     * @param sizeMax 上传总文件大小，默认不限制
     * @param fileSizeMax 单个文件上传大小，默认不限制
     */
    ...
    public void setAcceptTypes(String[] acceptTypes){
        ...
        for(String acceptType : acceptTypes){
            this.acceptTypes.add(acceptType.toLowerCase());
        }
    }
    ...
    public String upload(HttpServletRequest request,MultipartFile file){
        ...
        try {
            ...
            String fileName = fileNameGenerator.generate(suffix);
    ...
    private Result parseFileField(FileItem item, String absolutePath, FileNameGenerator fnGenerator, List<FileItemInfo> fiis)
    {
        ...
        String saveFileName = fnGenerator.generate(suffix);
    ...
    private void parseFormField(FileItem item, String encoding)
    {
        ...
        if(StringUtils.isNotEmpty(value) && encoding != null)
        {
            try
            {
    ...
    private String getAbsoluteSavePath(HttpServletRequest request)
    {
        ...
        if(!file.isAbsolute())
            file = new File(HttpUtil.getRequestRealPath(request, savePath));
        if(file.isDirectory())
