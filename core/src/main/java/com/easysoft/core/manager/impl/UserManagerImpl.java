...
public class UserManagerImpl implements IUserManager {
	...
	public Integer createUser(JEAPUser user) {
		//检测用户是否已经存在
		String sql ="select count(0) from jeap_user where username=?";
	...
	public int login(String username, String password) {
		int result=0;
		try{
            JEAPUser user = this.get(username);
	...
	public int checkIsLogin() {
		WebSessionContext<JEAPUser> sessonContext = ThreadContextHolder
		.getSessionContext();
        JEAPUser user = sessonContext.getAttribute(IUserManager.USER_SESSION_KEY);
		if(user!=null)
			return 1;
		else
			return 0;
	...
	public JEAPUser getCurrentUser() {
		WebSessionContext<JEAPUser> sessonContext = ThreadContextHolder.getSessionContext();
        JEAPUser user = sessonContext.getAttribute(IUserManager.USER_SESSION_KEY);
		return user;
	}
	...
	public JEAPUser get(Integer userid) {
		...
		return (JEAPUser)this.daoSupport.queryForObject(sql, JEAPUser.class, userid);
	}
	
	

	public IDaoSupport<JEAPUser> getDaoSupport() {
		return daoSupport;
	}



	public void setDaoSupport(IDaoSupport<JEAPUser> daoSupport) {
	...
	public JEAPUser get(String username) {
		return  (JEAPUser)this.daoSupport.queryForObject("select * from jeap_user where username=?", JEAPUser.class, username);
	}
